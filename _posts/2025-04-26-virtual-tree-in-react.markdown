...
function useVirtualize({ getScrollableElem, itemCount, itemSize }) {
  ...
  const endIndex = Math.ceil((scrollTop + viewportHeight) / itemSize);
...
Then in a component, you would potentially use it like so:
      ...
      <div style={{ height: `${virtualList.totalHeight}px`, position: 'relative' }}>
        {virtualList.virtualizedItems.map((virtualItem) => (
          <div
            ...
            {tableData[virtualItem.index].label}
          </div>
        ))}
      </div>
    </div>
...
3. Any time your data changes size or count or the viewport changes size, you have to re-virtualize
...
Before we look at how our code would need to change to accommodate trees, let's talk about the shape of tree data. For simplicity, I'm only going to talk about two different shapes of tree data; one is be nested and the other will be flat. Here's an example of a nested tree structure:
...
Awesome, so now we are producing a list of items in "tree order". This is exactly what is needed to pass to a virtualizer, so let's add this into the example component at the beginning of this post:
      ...
      <div style={{ height: `${virtualList.totalHeight}px`, position: 'relative' }}>
        {virtualList.virtualizedItems.map((virtualItem) => (
          <div
            ...
            {flattenedTree[virtualItem.index].label}
          </div>
        ))}
      </div>
    </div>
...
const TreeNode = ({ nodeData, level }) => {
  ...
  return (
    <>
      <div style={{ paddingLeft: `${level * 16}px` }}>{nodeData.label}</div>
